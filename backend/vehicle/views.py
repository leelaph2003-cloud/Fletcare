...
# FIX 1: User type checking functions defined ONCE only (removed duplicate definitions below)
def is_admin(user):
...
def customer_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            # FIX 2: Initialize KNN location only after customer.save() so id exists
            try:
...
def admin_dashboard_view(request):
    ...
    default_admin_lat = 12.9716  # Bangalore coordinates
...
def update_customer_view(request,pk):
    ...
    # FIX 3: Correct form initialization - pass no data for GET request
...
def admin_add_customer_view(request):
    ...
    if request.method=='POST':
        ...
        # FIX 10: Removed misplaced return that caused redirect even on invalid form
...
# FIX 7: Only ONE delete_mechanic_view - removed duplicate. This version redirects to admin-view-mechanic (correct)
@login_required(login_url='adminlogin')
...
def update_mechanic_view(request,pk):
    ...
    # FIX 4: Correct form initialization for GET request
...
def edit_customer_profile_view(request):
    ...
    user=models.User.objects.get(id=customer.user_id)
    userForm=forms.CustomerUserForm(instance=user)
    # FIX 5: Correct form initialization for GET request
    customerForm=forms.CustomerForm(instance=customer)
...
# FIX 8: Added @login_required to all three invoice views
@login_required(login_url='customerlogin')
@user_passes_test(is_customer)
    ...
    try:
        ...
        for service_request in completed_requests:
            try:
                ...
                failed_count += 1
                error_messages.append(f"Request #{service_request.id}: {str(e)}")
        ...
        if failed_count > 0:
            ...
            messages.warning(request, 'To fix email issues: Check your Gmail App Password in settings.py.')
...
def edit_mechanic_profile_view(request):
    ...
    user=models.User.objects.get(id=mechanic.user_id)
    userForm=forms.MechanicUserForm(instance=user)
    # FIX 6: Correct form initialization for GET request
    mechanicForm=forms.MechanicForm(instance=mechanic)
    ...
    if request.method == 'POST':
        if 'update_customer' in request.POST:
            ...
            try:
                ...
                customer.save()
                # FIX 9: get_name is a @property, call as attribute not method
